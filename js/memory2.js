const app = {
  ...
  generateVerso(number) {
    ...
    for (let index = 0; index < number; index++) {
      ...
      card.classList.add("recto");
      sectionElement.appendChild(card);
      const img = document.createElement("img");
      img.src = app.fruits[app.rectos[index]];
      img.style.width = "100%";
          ...
          if (app.nbCardReturn === 2) {
            ...
            setTimeout(() => {
              if (app.verifyPair()) {
                ...
                tryButton.value = `Erreurs : ${app.try}`;
              }
            }, 800);
          }
        }
  ...
  runElement() {
    ...
    runButton.addEventListener("click", () => {
      app.chronoUp = true;
      app.chronoDown = false;
      const nbButton = document.querySelector(".parameters__nb-pairs-input");
      const nbDifficulty = document.querySelectorAll(".difficulty");
      nbDifficulty.forEach((element) => {
        if (element.checked) {
          app.difficulty = element.value;
        }
      });
      ...
      runBtn.classList.add("avoid-clicks");
      setTimeout(() => {
        const rectoCard = document.querySelectorAll(".recto");
        rectoCard.forEach((element) => {
          element.classList.remove("recto");
          element.classList.add("verso");
        });
        const img = document.querySelectorAll(".container__card img");
        img.forEach((element) => {
          element.src = "../image/memory/verso.png";
        });
        const btnTry = document.querySelector(".try");
        btnTry.value = "Erreurs :";
        app.chronoUp = false;
        card.forEach((element) => {
          element.classList.remove("avoid-clicks");
        });
  ...
  deleteAllElement() {
    ...
    allCard.innerHTML = `<div class="panel-winning hidden">
      ...
      <p class="winning-texte t2">Vous avez fait ${app.try} erreurs</p>
  ...
  chrono() {
    const btnTry = document.querySelector(".try");
    btnTry.value = app.difficulty;
    const id = setInterval(printChrono, 1000);
    function printChrono() {
      if (app.chronoUp && !app.chronoDown) {
        btnTry.value = btnTry.value - 1;
      } else if (!app.chronoUp && app.chronoDown) {
        clearInterval(id);
      }
    }
  },
